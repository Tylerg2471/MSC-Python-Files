...
#Errfile = open("Scen2R2V48.txt","a+")



R = Robot([10,10,0],[2,0,-0.5,0,10],0.5,[2,2,2,4,4])
...
S = SurfaceRobot(0,-0,2,90,0.5)
SA = SurfaceRobot(0,0,2,90,0.5)
SB = SurfaceRobot(0,0,2,90,0.5)
...
for i in range(0,5):
    ...
    for i in range(0,50):
        ...
        A.GradCalcGoal(EKF.state[0,0],EKF.state[1,0],1,1)
        A.CalcOutput()
        ...
        AB.GradCalcGoal(R.Odostate[0,0],R.Odostate[1,0],1,1)
        AB.CalcOutput()
...
EkErrorx = R.state[0,0] - EKF.state[0,0]
...
#Errfile.close()
print(R.state[0,0], EKF.state[0,0])
print("done")